function [Seizure_start, Seizure_end, Seizure_ref_time] = Seizure_Detector2(data,window_length,sampling_frequency,Window_number,Start,interval_duration,threshold,Animal_number,thresholdAmp)
...
if (((strcmp(Start.AMPM,'PM')) && (Start.Hours~=12))) % Check if time is in AM or at 12PM
    Start.Hours = Start.Hours +12;
end
...
if Seizure(1) ==1
    n= n+1;% Increment the number of seizure starts
    Seizure_start_time_sec(n) = Window_time+Seizure_time(1);% Specify start of seizure at beginning of window plus the time of the transition
end
