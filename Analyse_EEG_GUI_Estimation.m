...
Annotate_dir = DetectorSettings.ExcelFilepath;
if ((length(EEGDir) ~= length(Annotate_dir)) && ProgramType(2))
    err=1;
    return
end
for direct = 1:length(EEGDir)
    ...
    try
        [fs, ~,~,~,~,StartDateTime,Start.StudyLength,ChannelName] = CMConnect_ProFusionEEG4(DetectorSettings.EEGFilepath); % Determine study frequency (fs), the time and date the study started (StartDateTime), its length (StudyLength) and the names of channels (ChennelName)
    catch
        [fs, ~,~,~,~,StartDateTime,Start.StudyLength,ChannelName] = CMConnect_ProFusionEEG41(DetectorSettings.EEGFilepath);
    end
    ...
    if (any(ProgramType) || any(EstimatorType))
        ...
        if (ProgramType(2) || EstimatorType(2)) % Seizure Characterisation is specified
            ...
            Number_of_windows = floor(Start.StudyLength/interval_duration); % Specify number of windows to analyse. Notice that by doing this the last data segment < interval_duration in length is lost
