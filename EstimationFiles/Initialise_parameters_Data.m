...
if Dp >0
    Parameter_std_deviation(1,1) = max(Max_A-Initial_excitability,Initial_excitability-Min_A)/number_of_sigma*std_adjustment_parameters; % Variance to allow for tracking of parameters Parameters appear to work  5e-3
    if Dp >1
        Parameter_std_deviation(1,2) = max(Max_B-Initial_inhibition,Initial_inhibition-Min_B)/number_of_sigma*std_adjustment_parameters;
        if Dp >2
            Parameter_std_deviation(1,3) = max(Max_G-Initial_Finhibition,Initial_Finhibition-Min_G)/number_of_sigma*std_adjustment_parameters;
        end
    end
else
...
if Dk==1
    ...
    Input_std_deviation = max((X(Ds+1) - frequency_limits(1)),frequency_limits(2)-X(Ds+1))/std_adjustment_parameters; % Define the variance of the input to the model Base 1e-2
...
if Dp >0                                    % Check whether parameters need to be estimated, if so alter storage folder
    ...
    if Dp >1                                % Check whether more than one parameter needs to be estimated
        ...
        if Dp==3                            % Check if parameter G needs to be estimated
            X(Ds+Dk+3,1) = Initial_Finhibition;% Set the intial guess for the G to be the initial value specified.
        end
    end
else
