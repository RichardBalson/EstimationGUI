...
t = linspace(0,dt*length(Y),length(Y));
tz = linspace(tstart,tstart+zoom,zoom*sampling_frequency+1);

index = {1:length(Y) tstart*sampling_frequency+1:(tstart+zoom)*sampling_frequency+1};

if (Image_handling_model_output(1,1))
    
    legendT = {'Obs.','Est.'};
    NMO=state_figure('Wendling Neural Mass Output','Obs',fig_settings,t,[Y(index{1})'; X(2,index{1})-X(3,index{1})],legendT,[]);
...
if (Image_handling_model_output(2,1))
    ...
    legendT = {'Obs.','Est.'};
    NMO=state_figure('Wendling Neural Mass Output Zoomed In','Obs',fig_settings,tz,[Y(index{2})'; X(2,index{2})-X(3,index{2})],legendT,[]);
    
end
legendT = {'Est. v_{p0}','Std. Dev.';...
    'Est. v_{p1}','Std. Dev.';...
    'Est. v_{p2}','Std. Dev.';...
    'Est. v_{p3}','Std. Dev.';...
    'Est. z_{p0}','Std. Dev.';...
...
for k = 1:Ds
    ...
    if (Image_handling_states(1,k))
        ...
        NMS(k)=state_figure(fig_name{k},'State',fig_settings,t,[X(k,index{1})],legendT(k,:),[erfn;erfp]);
...
for k = 1:Ds
    ...
    if (Image_handling_states(2,k))
        ...
        NMSZ(k)=state_figure(fig_name{k},'State',fig_settings,tz,[X(k,index{2})],legendT(k,:),[erfn;erfp]);
...
if Dk
    if (Image_handling_states(1,9))
        ...
        NMS(Ds+Dk)=state_figureInput('Neural Mass (Input)','State',fig_settings,t,[X(Ds+Dk,index{1})],{'Est. Input','Std. Dev.'},[erfn;erfp]);
    ...
    if (Image_handling_states(2,9))
      if (plot_uncertainty)  
        ...
        NMSZ(Ds+Dk)=state_figureInput('Neural Mass (Input)','State',fig_settings,tz,[X(Ds+Dk,index{2})],{'Est. Input','Std. Dev.'},[erfn;erfp]);
    ...
    for k = 1:Dp
        if (Image_handling_states(1,Ds+1+k))
           if (plot_uncertainty)  
            ...
            NMS(Ds+Dk+k)=state_figure(fig_name{k},'State',fig_settings,t,[X(Ds+Dk+k,index{1})],{'Est. Input','Std. Dev.'},[erfn;erfp]);
        end
        if (Image_handling_states(2,Ds+1+k))
            ...
            NMSZ(Ds+Dk+k)=state_figure(fig_name{k},'State',fig_settings,tz,[X(Ds+Dk+k,index{2})],{'Est. Input','Std. Dev.'},[erfn;erfp]);
        end
    end
end
clear erfn erfp
...
if any(Image_handling_inputs(1,:))
    Input(:,:,1) = [ExpY(index{1})];
    Input(:,:,2) = [C(1)*X(1,index{1})];
    Input(:,:,3) = [C(3)*X(1,index{1})];
    Input(:,:,4) = [C(5)*X(1,index{1})-C(6)*X(3,index{1})];
end
for k = Ds/2
    if (Image_handling_inputs(1,k))
        NMSI(k)=state_figure(fig_name{k},'State',fig_settings,t,Input(:,:,k),{'Est. Input'},[]);
    end
end
if any(Image_handling_inputs(2,:))
    clear Input
    Input(:,:,1) = [ExpY(index{2})];
    Input(:,:,2) = [C(1)*X(1,index{2})];
    Input(:,:,3) = [C(3)*X(1,index{2})];
    Input(:,:,4) = [C(5)*X(1,index{2})-C(6)*X(3,index{2})];
end
for k = Ds/2
    if (Image_handling_inputs(2,k))
        NMSIZ(k)=state_figure(fig_name{k},'State',fig_settings,tz,Input(:,:,k),{'Est. Input'},[]);
    end
end
clear Input
...
if (Image_handling_multi(1,1) ==1) % Plot all model states
    ...
    PlotsPerFig =1;
    for k = 1:Ds
        input((k-1)*PlotsPerFig+1:k*PlotsPerFig,:) =[X(k,index{1})];
        if plot_uncertainty
    ...
    NMM(1) = state_figure_multi(fig_name,'State',fig_settings,t,input,{'Est.'},[erfn;erfp],RowP,ColP,namesS,PlotsPerFig);
...
if (Image_handling_multi(2,1) ==1) % Plot all model states
    ...
    PlotsPerFig =1;
    for k = 1:Ds
        input((k-1)*PlotsPerFig+1:k*PlotsPerFig,:) =[X(k,index{2})];
        if plot_uncertainty
...
if ((Image_handling_multi(1,3) ==1) &&(Dp+Dk >0)) % Plot model parameters estimated
    ...
    PlotsPerFig =1;
    for k =1:Dp
    input((k-1)*PlotsPerFig+1:k*PlotsPerFig,:) = [X(Ds+Dk+k,index{1})];
        if plot_uncertainty
    ...
    if Dk 
        input(Dp*PlotsPerFig+1:(Dp+1)*PlotsPerFig,:)= [X(Ds+1,index{1})];
        if plot_uncertainty
...
if (Image_handling_multi(2,3) ==1) % Plot model parameters estimated
    ...
    PlotsPerFig =1;
    for k =1:Dp
    input((k-1)*PlotsPerFig+1:k*PlotsPerFig,:) = [X(Ds+Dk+k,index{2})];
        if plot_uncertainty
    ...
    if Dk 
        input(Dp*PlotsPerFig+1:(Dp+1)*PlotsPerFig,:)= [X(Ds+1,index{2})];
        if plot_uncertainty
...
if (Image_handling_firing_rates(3) ==1)
    ...
    input = [C(3)*X(1,index{1});...
            sigmoid(C(3)*X(1,index{1}));...
            X(3,index{1})];
    fig_name = 'Neural Mass Excitatory Neuron Input, Output and Firing Rate';
...
if (Image_handling_firing_rates(4) ==1)
    ...
    ColP=1;
    PlotsPerFig =1;
        input = [C(5)*X(1,index{1})-C(6)*X(3,index{1});...
            sigmoid(C(5)*X(1,index{1})-C(6)*X(3,index{1}));...
            X(4,index{1})];
